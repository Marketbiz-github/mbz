    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-1 flex items-center gap-2">
            Client: <span className="text-emerald-400 font-semibold">{project?.clients?.name}</span>
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="high-tech-card p-12 border-dashed border-white/10 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-400 max-w-md mb-6">You haven't sent any WhatsApp blasts for this project. Create your first campaign to get started.</p>
          <Link
            href={`/wa-blast/create/${projectId}`}
            className="bg-white/10 hover:bg-white/15 text-white px-6 py-3 rounded-lg font-bold text-xs transition-colors cursor-pointer"
          >
          ...
          {filteredReports.map((report) => (
            <div key={report.id} className="high-tech-card bg-slate-900/40 p-0 overflow-hidden flex flex-col border border-white/5 hover:border-emerald-500/30 transition-all group">
              ...
              <div className="p-4 border-t border-white/5 bg-slate-900/60">
                <Link 
                  href={`/wa-blast/report/${report.id}`}
